...
class nzmodel():
    ...
    def NetZeroFunDamage(self, t, E0, E0_ref, xin, yin, zin, D, effective_GMT, limit_closest, ind_closest, switch_unc, care):
        ...
        kappa_know = (effective_GMT/limit_closest)**alpha
        ...
        for tt in range(Tf_start, Tf_end):
            ...
            z_i = z_arr[i-1] + self._tau11*(1-D_arr[i]/(1+D_arr[i]))*(D_arr[i]-self._mu1*y_arr[i-1] + kappa_know*care_perceived*global_functions.unc_limits(ind_closest, switch_unc))
            z_arr.append(z_i)
